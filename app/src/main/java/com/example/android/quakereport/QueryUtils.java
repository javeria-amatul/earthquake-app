...
public final class QueryUtils {
     ...
     */
    public static ArrayList<Earthquake> parseEarthquakesResponse(String JSON_RESPONSE) {
        ...
        try {
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                String magnitutde = Double.toString(propertiesObj.getDouble("mag"));
                String location = propertiesObj.getString("place");
                Long timeStampInMillis = propertiesObj.getLong("time");
                SimpleDateFormat formatter = new SimpleDateFormat("dd/MM/yyyy hh:mm:ss", Locale.getDefault());
                Calendar calendar = Calendar.getInstance();
                calendar.setTimeInMillis(timeStampInMillis);
                String timestamp = formatter.format(calendar.getTime());
                Earthquake earthquake = new Earthquake(magnitutde, location, timestamp);
    ...
    public static URL createUrl(String stringUrl) {
        ...
        try {
            ...
            Log.e(TAG, "Error with creating URL",ex);
        }
        return url;
    }
